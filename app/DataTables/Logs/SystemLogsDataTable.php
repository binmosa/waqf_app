...
class SystemLogsDataTable extends DataTable
{
    ...
    {
        return [
            Column::make('id')->title('رقم العملية')->width(50),
            Column::make('message')->title('الرسالة'),
            Column::make('level')->title('نوع العملية'),
            Column::make('date')->width(130)->title('وقت العملية'),
            Column::computed('action')
                ->exportable(false)
                ->printable(false)
                ->addClass('text-center')
                ->responsivePriority(-1),
            Column::make('environment')->addClass('none')->title('بيئة العملية'),
            Column::make('file_path')->title(__('مسار العملية'))->addClass('none'),
            Column::make('context')->addClass('none')->title('تفاصيل العملية'),
        ];
    }
