<x-base-layout>
    <!--begin::Content-->
    <x-admin.tables.table1 class="card-xxl-stretch mb-5 mb-xl-8 table-responsive" title="جميع المتطوعين"
        userCount="اكثر من {{ count($volunteers) }} متطوع"  :columns="['اسم الكامل', 'البريد الالكتروني', 'نوع التطوع', 'ملاحظة', 'السجلات','خيارات']">
        @foreach ($volunteers as $row)
            <tr>
                ...
                <td>
                    ...
                    @hasrole('superadmin')
                    <form method="post" class="d-inline-block" action="{{ route('volunteers.destroy', $row->id) }}">
                        @csrf
                        @method('DELETE')
                        {{-- <input type="hidden" value="{{ $row->id }}" name="id"> --}}
                        <a href="{{ route('volunteers.destroy', $row->id) }}"
